...
class Lock(commands.Cog):
    ...
    async def lockset_ignore(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        if channel.id not in await self.config.guild(ctx.guild).ignore():
            ...
            return await ctx.send(f"{channel.mention} has been added into the ignored channels list.")
        ...
        async with ctx.typing():
            ...
            for channel in ctx.guild.text_channels:
                ...
                if which:
                    ...
                    await channel.set_permissions(ctx.guild.default_role, read_messages=False, send_messages=True)
